...
int AdjPrefetch::prefetchMiss(uint64_t address, unsigned int tid, std::array<int,64> data, std::string precomp_method, std::string precomp_update_method, std::string comp_method, \
    int entries, std::string infinite_freq, int frequency_threshold, std::string hit_update, std::string ignore_i_bytes, int data_type, int bytes_ignored, int sim_threshold, System& sys)
{
   sys.memAccess(address + (1 << sys.setShift), AccessType::Prefetch, data, tid, precomp_method, precomp_update_method, comp_method, entries, infinite_freq, frequency_threshold, hit_update, \
    ...
    int entries, std::string infinite_freq, int frequency_threshold, std::string hit_update, std::string ignore_i_bytes, int data_type, int bytes_ignored, int sim_threshold, System& sys)
{
   ...
   if(tag == lastTag && (lastSet+1) == set) {
      for(uint64_t i=0; i < prefetchNum; i++) {
         ...
         sys.memAccess(address + ((1 << sys.setShift) * (i+1)), AccessType::Prefetch, data, tid, precomp_method, precomp_update_method, comp_method, entries, infinite_freq, frequency_threshold, \
    ...
    int entries, std::string infinite_freq, int frequency_threshold, std::string hit_update, std::string ignore_i_bytes, int data_type, int bytes_ignored, int sim_threshold, System& sys)
{
   ...
   if(tag == lastTag && lastSet == set) {
      ...
      // incremented in the set portion of its bits (least
      // significant bits not in the cache line offset portion)
      sys.memAccess(address + ((1 << sys.setShift) * prefetchNum), AccessType::Prefetch, data, tid, precomp_method, precomp_update_method, comp_method, entries, infinite_freq, frequency_threshold, \
        hit_update, ignore_i_bytes, data_type, bytes_ignored, sim_threshold);
